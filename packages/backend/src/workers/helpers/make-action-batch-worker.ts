...
async function processBatch(ctx: BatchContext): Promise<void> {
  ...
  // withLock's `finally` after the write + bookkeeping. A null key means the
  // app declares no lock, so the batch runs directly.
  ...
  await withLock(lockKey, runBatchAndFinalize, {
    span,
