...
const updateStep: MutationResolvers['updateStep'] = async (
  ...
  const step = await Step.transaction(async (trx) => {
    ...
    // Validate + accept a client-supplied config.endStepId (only for a valid
    // if-then target); returns {} when the key is absent, so the config spread
    // below preserves any existing marker. Throws (rolls back) on a bad target.
