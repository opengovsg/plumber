...
import { redactVariables } from '@/graphql/mutations/duplicate-branch.redact'

describe('duplicateBranch redactVariables', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  ...
  it('hands every step to the step redactor', () => {
    const steps = [{ key: 'httpRequest' }, { key: 'plainAction' }]

    const result = redactVariables({
      input: { flow: { id: 'flow-1' }, steps },
    })
  ...
  it('leaves previousStep alone, because it carries no parameters', () => {
    const result = redactVariables({
      input: { previousStep: { id: 'step-0' }, steps: [] },
    })
