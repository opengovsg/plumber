 ...
 */
export function parseOperand(format: SupportedFormat, value: string): DateTime {
  const parsed = parseDateTime(format, value)

  const result = ZONELESS_FORMATS.has(format)
    ? parsed.setZone(SG_TIMEZONE, { keepLocalTime: true })
    : parsed.setZone(SG_TIMEZONE)

  if (!result.isValid) {
    throw new Error(
      `Could not convert '${value}' to Singapore time: ${result.invalidReason}`,
    )
  }
...
export function computeDifference(
  ...
  return { result: signed, absolute, unit }
}
