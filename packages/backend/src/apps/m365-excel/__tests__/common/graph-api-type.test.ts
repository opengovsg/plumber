...
import { getGraphApiType, GraphApiType } from '../../common/graph-api-type'

describe('graph api type helper function', () => {
  it.each([
    ...
    expect(getGraphApiType(url)).toEqual(GraphApiType.Excel)
  })
  ...
  it.each([
    ...
    expect(getGraphApiType(url)).toEqual(GraphApiType.SharePoint)
  })

  it.each(['/v1.0/auth', '/v1.0/me', 'https://graph.microsoft.com/v1.0/me'])(
    'classifies other endpoints as unknown',
    (url) => {
      expect(getGraphApiType(url)).toEqual(GraphApiType.Unknown)
    },
  )
})
