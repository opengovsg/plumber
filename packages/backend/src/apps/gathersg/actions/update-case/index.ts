import type { IGlobalVariable, IJSONArray, IRawAction } from '@plumber/types'
...
import { GATHER_ATTACHMENT_FIELD_TYPE } from '../../common/constants'
...
const action: IRawAction = {
  ...
  doesFileProcessing: (step: Step) =>
    ((step.parameters.attachmentUpdates as IJSONArray | undefined) ?? []).some(
      (row) =>
        Array.isArray((row as IJSONArray).attachments) &&
        ((row as IJSONArray).attachments as IJSONArray).length > 0,
    ),
    ...
    try {
      ...
      if (attachmentUpdates.length > 0) {
        const attachmentFlagValue = await getLdFlagValue(
          getInputFlag('updateCase', 'attachmentUpdates'),
          $.user?.email ?? null,
          null,
        )
        if (
