...
describe('deriveIfThenV1EndStep', () => {
  it('derives the extent of a 2-branch block up to the step before the next if-then', () => {
    // trigger, ifThenA, stepA, ifThenB, stepB
    ...
    // First block ends at the step before the second if-then.
    expect(deriveIfThenV1EndStep(steps, ifThenA).id).toBe('stepA')
    // Last block runs to the end of the flow.
  ...
  it('treats a following (reject-branch) if-then as an MRF boundary', () => {
    // A non-approval if-then, a step, then an MRF submission and a
    // reject-branch if-then. The reject-branch if-then still bounds the extent.
    const ifThenA = ifThen('ifThenA', 2)
    const rejectIfThen = ifThen('rejectIfThen', 5, {
      // approval-bearing, but still a depth-0 if-then boundary
      parameters: { depth: '0' },
    })
    const steps = [
      trigger(1),
