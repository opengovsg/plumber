...
describe('getIfThenV2BlockSkipSteps', () => {
  it('returns the block if-then for a step inside its block', () => {
    ...
    expect(getIfThenV2BlockSkipSteps(flowSteps, child)).toEqual([ifThen])
  })
  ...
  it('includes every only-continue-if inside the block, ordered by position', () => {
    ...
    expect(getIfThenV2BlockSkipSteps(flowSteps, lastChild)).toEqual([
      ifThen,
      firstOci,
      secondOci,
    ])
  ...
  it('returns the if-then for the block endStep itself', () => {
    ...
    expect(getIfThenV2BlockSkipSteps(flowSteps, endStep)).toEqual([ifThen])
  })
  ...
  it('picks only the containing block when a flow has several', () => {
    ...
    expect(getIfThenV2BlockSkipSteps(flowSteps, secondChild)).toEqual([
      secondIfThen,
    ])
  })
  ...
  it.each([
    ...
    expect(getIfThenV2BlockSkipSteps(steps, step)).toEqual([])
  })
})
