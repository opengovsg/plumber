...
const trigger: IRawTrigger = {
  ...
  webhookTriggerInstructions: {
    ...
    mockDataMsg:
      'The data below is mock data generated by Plumber. Make a FormSG submission to replace the data. ',
  },
  arguments: [
    {
      label: 'NRIC Filter',
  ...
  async testRun($: IGlobalVariable) {
    ...
    // if different or no form is detected, it is a mock run
    const isMockRun = lastExecutionStep?.dataOut?.formId !== formId

    // if different or no form is detected, use mock data
    await $.pushTriggerItem({
      raw: isMockRun ? await getMockData($) : lastExecutionStep?.dataOut,
      meta: {
        internalId: '',
      },
      mockRun: isMockRun ? true : lastExecutionStep.mockRun, // use previous mock run status by default
    })
  },
}
