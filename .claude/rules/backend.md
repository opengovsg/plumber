...
GraphQL schema and resolvers also live here ([packages/backend/src/graphql/](packages/backend/src/graphql/)). `graphql-shield` handles authz, `graphql-rate-limit` rate-limits. The frontend code is documented in [.claude/rules/frontend.md](.claude/rules/frontend.md).
